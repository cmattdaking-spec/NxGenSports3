...
export default function Settings() {
  ...
  const startProfileEdit = () => {
    setProfileForm({ full_name: user?.full_name || "", position: user?.position || "" });
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      setProfileMsg({ text: "Failed to upload photo.", type: "error" });
    }
  ...
  const saveProfile = async () => {
    if (!profileForm.full_name.trim()) {
      setProfileMsg({ text: "Name cannot be empty.", type: "error" }); return;
    }
    setProfileSaving(true);
    try {
      const updated = await base44.auth.updateMe({
        full_name:  profileForm.full_name.trim(),
        first_name: profileForm.full_name.trim().split(" ")[0] || "",
        last_name:  profileForm.full_name.trim().split(" ").slice(1).join(" ") || "",
        position:   profileForm.position.trim(),
      });
      setUser(updated);
      setEditingProfile(false);
      setProfileMsg({ text: "Profile saved!", type: "success" });
      setTimeout(() => setProfileMsg({ text: "", type: "" }), 2500);
    } catch {
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-full p-4 md:p-6 space-y-6 max-w-2xl mx-auto">
      ...
      {/* ── Profile Card — editable for all users ── */}
      {user && (
        <div className="bg-[#141414] border border-gray-800 rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {!editingProfile && (
              <button onClick={startProfileEdit}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-400 hover:text-white hover:bg-gray-800 transition-all">
          ...
          <div className="flex items-center gap-4">
            <div className="relative flex-shrink-0">
              ...
              {/* Camera overlay */}
              <label className="absolute inset-0 rounded-full flex items-center justify-center bg-black/50 opacity-0 hover:opacity-100 transition-opacity cursor-pointer">
                {photoUploading
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-[var(--color-primary,#3b82f6)] capitalize font-medium">
                    {(user.coaching_role || user.role)?.replace(/_/g, " ")}
                  </span>
                </div>
              </div>
            ) : (
              <div className="flex-1 space-y-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#1a1a1a] border border-gray-700 rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-[var(--color-primary,#3b82f6)]"
                  />
                </div>
                <div>
                  <label className="text-gray-400 text-xs mb-1 block">Position / Title</label>
                  <input
          ...
          {editingProfile && (
            <div className="flex gap-2">
              <button onClick={saveProfile} disabled={profileSaving} data-testid="save-profile-btn"
          ...
          <p className="text-gray-600 text-xs">
            Tap your photo to change it. All users can update their name, title and photo.
          </p>
        </div>
      )}
