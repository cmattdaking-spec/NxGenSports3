...
import { Plus, Edit, Trash2, X, BookOpen, Brain, Calendar, MapPin, ChevronDown, ChevronUp, Target } from "lucide-react";
...
const EDIT_ROLES = ["admin","head_coach","associate_head_coach","offensive_coordinator","defensive_coordinator","special_teams_coordinator","strength_conditioning_coordinator","position_coach"];

export default function GamePlanTab({ user }) {
  ...
  const load = async () => {
    const [gp, op, pl, py] = await Promise.all([
    ...
    setPlans(gp); setOpponents(op); setPlayers(pl); setPlays(py); setLoading(false);
  };
  ...
  const inp = "w-full bg-[#1a1a1a] border border-gray-700 text-white px-3 py-2 rounded-lg text-sm outline-none";

  const save = async () => {
    setSaving(true);
    if (editing) await base44.entities.GamePlan.update(editing.id, form);
    else await base44.entities.GamePlan.create({ ...form, sport: activeSport });
    setShowForm(false); setSaving(false); load();
  };

  const remove = async (id) => { if (!confirm("Delete this game plan?")) return; await base44.entities.GamePlan.delete(id); setPlans(p => p.filter(x => x.id !== id)); };

  const playsByUnit = (plan) => cfg.gamePlanSections
    .map(sec => ({ label: sec.label, items: plan[sec.key], color: sec.color }))
    .filter(s => s.items?.length > 0);

  const openAdd = () => { setEditing(null); setForm({ unit: cfg.units[0], status: "draft", scripted_plays: [], red_zone_plays: [], third_down_plays: [], two_minute_plays: [], opening_script: [] }); setShowForm(true); };
    ...
    <div className="p-4 md:p-6">
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm">AI-powered game planning · {plans.length} plans</p>
        </div>
        {canEdit && tab === "plans" && (
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-xl text-white text-sm font-semibold" style={{ backgroundColor: "var(--color-primary,#f97316)" }}>
      ...
      <div className="flex gap-1 bg-[#141414] border border-gray-800 rounded-lg p-1 w-fit mb-6">
        {[{ id: "plans", label: "Game Plans" }, { id: "ai", label: "NxPlan AI" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.id === "ai" && <Brain className="w-3.5 h-3.5 inline mr-1.5" />}{t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "ai" && (
        <div className="space-y-4">
          <div>
            ...
            <select value={selectedOpponent?.id || ""} onChange={e => setSelectedOpponent(opponents.find(o => o.id === e.target.value) || null)}
          ...
          <NxPlanAI opponent={selectedOpponent} players={players} plays={plays} onSave={() => { load(); setTab("plans"); }} />
        </div>
      )}
        ...
        <div className="space-y-4">
          {plans.length === 0 ? (
            <div className="text-center py-20 text-gray-600">
              ...
              <button onClick={() => setTab("ai")} className="mt-4 px-4 py-2 rounded-xl text-white text-sm font-semibold" style={{ backgroundColor: "var(--color-primary,#f97316)" }}>
                ...
                <div className="px-5 py-4 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_CFG[plan.status]?.color || STATUS_CFG.draft.color}`}>{STATUS_CFG[plan.status]?.label || "Draft"}</span>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-gray-700/50 text-gray-300 capitalize">{plan.unit?.replace("_"," ")}</span>
                    </div>
                    <div className="flex items-center gap-4 text-gray-500 text-xs flex-wrap">
                      ...
                      {plan.location && <span className="flex items-center gap-1 capitalize"><MapPin className="w-3 h-3" /> {plan.location}</span>}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {canEdit && <>
                      <button onClick={() => openEdit(plan)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-800 text-gray-400 hover:text-white"><Edit className="w-3.5 h-3.5" /></button>
                      <button onClick={() => remove(plan.id)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-800 text-gray-400 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                    </>}
                    <button onClick={() => setExpanded(e => ({ ...e, [plan.id]: !e[plan.id] }))} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-800 text-gray-400 hover:text-white">
                        ...
                        <div key={sec.label} className="bg-[#1a1a1a] border border-gray-700 rounded-xl p-3">
                          ...
                          <ol className="space-y-1">
                            {sec.items.slice(0, 8).map((play, i) => (
                              <li key={i} className="text-gray-300 text-xs flex items-start gap-1.5"><span className={`font-bold flex-shrink-0 ${sec.color}`}>{i + 1}.</span> {play}</li>
                            ))}
                          </ol>
                        </div>
                      ))}
          ...
          <div className="bg-[#141414] border border-gray-700 rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-gray-400 text-xs mb-1 block">Opponent *</label><input value={form.opponent || ""} onChange={e => setForm({...form, opponent: e.target.value})} className={inp} /></div>
                <div><label className="text-gray-400 text-xs mb-1 block">Game Date *</label><input type="date" value={form.game_date || ""} onChange={e => setForm({...form, game_date: e.target.value})} className={inp} /></div>
                <div>
              ...
              <div>
                ...
                <textarea rows={2} value={form.key_tendencies || ""} onChange={e => setForm({...form, key_tendencies: e.target.value})} className={inp + " resize-none"} />
              </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={save} disabled={saving || !form.opponent || !form.game_date} className="flex-1 text-white py-2.5 rounded-xl text-sm font-semibold disabled:opacity-50" style={{ backgroundColor: "var(--color-primary,#f97316)" }}>
